...
class BlenderBotChat {
    ...
    getApiUrl() {
        ...
        if (isLocal) {
            ...
            // Fallback: Update this with your Cloudflare Tunnel URL
            // Format: https://ml101bot.yourdomain.com
            return 'https://YOUR_CLOUDFLARE_TUNNEL_URL/api';
        } else {
    ...
    checkApiUrl() {
        ...
        if (needsHttps) {
            ...
            console.warn('⚠�E�EHTTPS API URL not configured. See docs/HTTPS-SETUP.md');
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
                ...
                if (isHttps && apiIsHttp) {
                    ...
                    errorMessage = this.currentLanguage === 'ja'
                        ? 'セキュリチE��上�E琁E��により、HTTPSペ�EジからHTTP APIに接続できません。ローカルでチE��トする場合�E、http://localhost:8000 でペ�Eジを開ぁE��ください、E
                    ...
                    errorMessage = this.currentLanguage === 'ja'
                        ? 'APIサーバ�Eに接続できません。バチE��エンドサーバ�Eが起動してぁE��か確認してください、E
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        window.blenderBotChat = new BlenderBotChat();
    });
} else {
    window.blenderBotChat = new blenderBotChat();
}
